...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Check max unique users
    if (voucher.max_users) {
      if (!customer_id) {
        return {
          status: false,
          message: 'Voucher này yêu cầu đăng nhập để sử dụng',
        }
      }
      ...
      const hasUsedBefore = uniqueCustomers.some(
        id => id?.toString() === customer_id.toString()
      )

      if (uniqueCustomers.length >= voucher.max_users && !hasUsedBefore) {
        return {
          status: false,
          message: 'Voucher đã đạt giới hạn số lượng người dùng'
        }
      }
    }
