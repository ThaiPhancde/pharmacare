...
<template>
  <n-card class="bg-gray-50 border-l-4 border-primary shadow-md">
    ...
    <n-form ref="formRef" :model="form" :rules="rules" label-placement="top" class="grid grid-cols-2 gap-4">
      <!-- Basic Info -->
      <n-form-item label="Supplier" path="supplier">
        <n-select v-model:value="form.supplier" :options="supplierOptions" placeholder="Select supplier" filterable />
      </n-form-item>

      <n-form-item label="Date" path="date">
        <n-date-picker v-model:value="form.date" type="date" clearable :is-date-disabled="disablePastDates"
          value-format="timestamp" />
      </n-form-item>

      <n-form-item label="Invoice No" path="invoice_no">
        <n-input v-model:value="form.invoice_no" placeholder="Auto generated" disabled />
      </n-form-item>

      <n-form-item label="Payment Type" path="payment_type">
        <n-select v-model:value="form.payment_type" :options="paymentOptions" placeholder="Select payment type" />
      </n-form-item>
        ...
        <div v-for="(item, index) in form.items" :key="index"
          ...
          <div class="grid grid-cols-3 gap-4">
            <n-form-item :label="'Medicine ' + (index + 1)" :path="'items[' + index + '].medicine'">
              <n-select v-model:value="item.medicine" :options="medicineOptions" placeholder="Select medicine"
                filterable @update:value="(val) => handleMedicineSelect(val, index)" />
            </n-form-item>

            <n-form-item label="Batch ID" :path="'items[' + index + '].batch_id'">
              <n-input v-model:value="item.batch_id" placeholder="Enter batch ID" disabled />
            </n-form-item>

            <n-form-item label="Expiry Date" :path="'items[' + index + '].expiry_date'">
              <n-date-picker v-model:value="item.expiry_date" type="date" clearable
                :is-date-disabled="disablePastDates" />
            </n-form-item>

            <n-form-item label="Box Pattern" :path="'items[' + index + '].box_pattern'">
              <n-input v-model:value="item.box_pattern" placeholder="e.g. 10x10" />
            </n-form-item>

            <n-form-item label="Box Quantity" :path="'items[' + index + '].box_quantity'">
              <n-input-number v-model:value="item.box_quantity" :min="0" />
            </n-form-item>

            <n-form-item label="Unit Quantity" :path="'items[' + index + '].unit_quantity'">
              <n-input-number v-model:value="item.unit_quantity" :min="0" disabled
                :value="calculateUnitQuantity(item)" />
            </n-form-item>

            <n-form-item label="Supplier Price" :path="'items[' + index + '].supplier_price'">
      ...
      <div class="col-span-2 flex justify-end gap-4 mt-4">
        ...
        <n-button type="primary" @click="handleSubmit" class="bg-primary hover:bg-primary-dark">Save</n-button>
      </div>
    </n-form>
  </n-card>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    // Only fetch data if we're editing (have an ID)
    if (route.params?.id && isMounted.value) {
      await fetchData();
    }
  } catch (error) {
    if (isMounted.value) {
...
const handleSubmit = () => {
  if (!formRef.value || !isMounted.value) {
    console.warn("Form ref is not available or component is unmounted");
    return;
  }

  formRef.value.validate(async (errors) => {
    if (errors || !isMounted.value) {
      if (errors) {
        message.warning("Please fill in all required fields correctly", { duration: 3000 });
      }
      return;
    }
    ...
    try {
      ...
      const errorMsg = error?.message || "Failed to save purchase";
      message.error(errorMsg, { duration: 5000 });
    }
  });
};
...
const fetchData = async () => {
  if (!route.params?.id || !isMounted.value) return;
  try {
    const res = await fetch(`/api/purchase/${route.params.id}`);
    if (!res.ok) throw new Error(`Error: ${res.status}`);
    const response = await res.json();
    if (!response.data || !isMounted.value) {
      if (!isMounted.value) return;
    ...
    if (isMounted.value && data.supplier && data.supplier._id) {
      Object.assign(form.value, {
        ...
        items: (data.items || []).map((item) => ({
          medicine: item.medicine?._id || null,
          batch_id: item.batch_id || null,
          expiry_date: item.expiry_date || null,
          box_pattern: item.box_pattern || null,
