...
const mergedConfig = computed(() => {
  const val = {} as Record<string, ConfigItem>;
  for (const key in shapes.value) {
    const shape = shapes.value[key];
    const custom = props.fieldConfig?.[key] || {};
    val[key] = {
      ...shape,
      ...custom,
      options: custom?.options ?? shape.options, // Nếu custom không có options, dùng của shape
    };
  }
  return val;
});
  ...
  <component :is="formComponent" v-bind="formComponentProps">
    <slot name="customAutoForm" :fields="fields">
      <template v-for="(shape, key) of shapes" :key="key">
        ...
        >
          <AutoFormField
            :config="mergedConfig[key as keyof typeof mergedConfig]"
            :field-name="key.toString()"
            :shape="shapes[key]"
          />
        </slot>
