...
- `max_users`: Giới hạn số lượng khách hàng được phép sử dụng
...
- Khách hàng không còn được tạo thủ công. Khi gửi `customer_details` trong payload `/api/invoice`, backend sẽ tìm theo `full_name/phone/email` và tự tạo record nếu chưa tồn tại.
- POS (`pages/invoice/pos.vue`) cũng có ô nhập voucher, gọi cùng logic kiểm tra `max_users`, `usage_limit`, và commit usage chung transaction với hoá đơn.
