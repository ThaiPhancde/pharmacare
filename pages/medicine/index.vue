...
<template>
  <div class="w-full flex flex-col items-stretch gap-4">
    ...
    <DataTable :data="dataTable" :columns filterKey="name" v-model:page="pagination.page" @changePage="handlePageChange"
      v-model:size="pagination.limit" v-model:total="pagination.total" />
      ...
      <div v-if="selectedMedicine" class="grid gap-4 py-4">
        ...
        <div v-if="selectedMedicine.bar_code" class="mt-2 p-3 border rounded-md">
          ...
          <div class="flex flex-col items-center">
            <BarcodeGenerator :value="selectedMedicine.bar_code" :height="80" :width="2" :display-value="true"
    ...
    <!-- Modal add -->
    <n-modal v-model:show="showAdd" close-on-esc preset="card" style="width: 90%; max-width: 900px"
      :title="selectedMedicine ? 'Edit Medicine' : 'Add New Medicine'">
      <n-form ref="formRef" :model="formValue" label-placement="top" class="space-y-4">
        <!-- Basic Information Section -->
        <div class="border-b pb-4 mb-4">
          <h3 class="text-lg font-semibold text-gray-700 mb-4">Basic Information</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <n-form-item label="Medicine Name" path="name" :required="true">
              <n-input v-model:value="formValue.name" placeholder="Enter medicine name" class="w-full" />
            </n-form-item>
            <n-form-item label="Generic Name" path="generic">
              <n-input v-model:value="formValue.generic" placeholder="Enter generic name" class="w-full" />
            </n-form-item>
            <n-form-item label="Strength" path="strength">
              <n-input v-model:value="formValue.strength" placeholder="e.g., 500mg, 10ml" class="w-full" />
            </n-form-item>
            <n-form-item label="Bar Code/QR Code" path="bar_code">
              <div class="flex gap-2">
                <n-input v-model:value="formValue.bar_code" placeholder="Scan or enter barcode" class="flex-1" />
  ...
  :style="{
    backgroundColor: '#3b82f6',  // 🔵 xanh đậm
    color: '#fff',
    border: 'none'
  }"
>
  Generate
              ...
              <div v-if="formValue.bar_code" class="mt-3 p-3 border rounded bg-gray-50">
                ...
                <BarcodeGenerator :value="formValue.bar_code" :height="60" :width="1.5" class="w-full" />
              </div>
            </n-form-item>
          </div>
        </div>
        ...
        <div class="border-b pb-4 mb-4">
          <h3 class="text-lg font-semibold text-gray-700 mb-4">Classification</h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <n-form-item label="Category" path="category_id" :required="true">
              <n-select clearable filterable :fallbackOption="false" v-model:value="formValue.category_id"
                :options="categories" placeholder="Select category" class="w-full" />
            </n-form-item>
            <n-form-item label="Unit" path="unit_id" :required="true">
              <n-select clearable filterable :fallbackOption="false" v-model:value="formValue.unit_id" :options="units"
                placeholder="Select unit" class="w-full" />
            </n-form-item>
            <n-form-item label="Type" path="type_id">
              <n-select clearable filterable :fallbackOption="false" v-model:value="formValue.type_id"
                :options="medicineTypes" placeholder="Select type" class="w-full" />
            </n-form-item>
          </div>
        </div>
        ...
        <div class="border-b pb-4 mb-4">
          <h3 class="text-lg font-semibold text-gray-700 mb-4">Pricing & Supplier</h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <n-form-item label="Supplier" path="supplier">
              <n-input v-model:value="formValue.supplier" placeholder="Enter supplier name" class="w-full" />
            </n-form-item>
            <n-form-item label="Supplier Price" path="supplier_price">
              <n-input-number class="w-full" clearable v-model:value="formValue.supplier_price" :precision="0" :min="0"
                placeholder="Enter supplier price" />
            </n-form-item>
            <n-form-item label="Selling Price" path="price" :required="true">
              <n-input-number class="w-full" clearable v-model:value="formValue.price" :precision="0" :min="0"
        ...
        <div class="border-b pb-4 mb-4">
          ...
          <div class="grid grid-cols-1 gap-4">
            <n-form-item label="Description" path="description">
              <n-input v-model:value="formValue.description" type="textarea" :rows="3" placeholder="Enter description"
                class="w-full" />
            </n-form-item>
            <n-form-item label="Product Image" path="image">
              <n-upload :show-file-list="true" @update:file-list="handleChangeFile" v-model:default-file-list="fileList"
                list-type="image-card" :max="1" :on-success="handleImageUpload" :before-upload="beforeImageUpload">
                <div
                  class="flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-lg hover:border-primary transition-colors">
        ...
        <div class="pb-4">
          <h3 class="text-lg font-semibold text-gray-700 mb-4">Sales Restrictions</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <n-form-item label="Prescription Required" path="prescription_required">
              <div class="flex items-center gap-3">
                <n-switch v-model:value="formValue.prescription_required" />
                <span class="text-sm text-gray-600">Require prescription when selling</span>
              </div>
            </n-form-item>
            <n-form-item label="Daily Purchase Limit" path="max_quantity_per_day">
              <n-input-number class="w-full" clearable v-model:value="formValue.max_quantity_per_day" :precision="0"
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t">
          <n-button @click="showAdd = false" size="large">Cancel</n-button>
