        ...
        <n-card title="Summary" class="mt-4 border-t-4 border-primary">
          <div class="flex justify-end">
            <div class="grid grid-cols-2 gap-4 min-w-[300px]">
              ...
              <div class="font-bold text-primary">{{ formatVND(calculateTotal) }}</div>
            </div>
          </div>
        </n-card>
      </div>
...
const {
  ...
  const res = await fetch("/api/medicine?" + new URLSearchParams({
    page: page.toString(),
...
const generateInvoiceNo = async () => {
  try {
    ...
    const res = await fetch("/api/purchase?" + new URLSearchParams({
      page: "1",
      limit: "1",
...
const rules = {
  supplier: { required: true, message: "Please select supplier", trigger: "blur" },
  date: { 
    required: true, 
  ...
  invoice_no: { required: true, message: "Please enter invoice number", trigger: "blur" },
  payment_type: { required: true, message: "Please select payment type", trigger: "change" },
  "items": {
    type: "array",
    required: true,
  ...
  "items[].batch_id": { required: true, message: "Batch ID is required", trigger: "blur" },
  "items[].expiry_date": { required: true, message: "Please select expiry date", trigger: "change" },
  "items[].box_pattern": { 
    required: true, 
  ...
  formRef.value?.validate(async (errors) => {
    ...
    try {
      ...
      if (result.status) {
        ...
        console.error('Purchase creation failed:', result.error || result.message);
