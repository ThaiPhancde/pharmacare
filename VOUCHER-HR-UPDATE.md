...
- ✅ Điều kiện áp dụng:
  ...
  - Giới hạn số lượng khách hàng được phép sử dụng
...
#### Customer Workflow (Mới)
- Trang khách hàng chuyển sang chế độ chỉ đọc, bỏ nút **Add Customer**.
- Khi tạo hoá đơn (kể cả POS), nếu nhập khách mới hệ thống tự tạo `customer_id` (KHxxxxx) và lưu liên hệ.
- Từ lần sau chỉ cần gõ tên/điện thoại để auto-complete, không phải nhập lại.

#### Voucher trong màn tạo đơn & POS
- `pages/invoice/add.vue` và `pages/invoice/pos.vue` có ô nhập mã giảm giá, hiển thị trạng thái áp dụng.
- Khi giỏ hàng thay đổi sau khi áp voucher, UI cảnh báo và yêu cầu apply lại để tránh lệch lượt dùng.
- Backend khóa voucher trong chính transaction, đảm bảo usage_count/max_users được cập nhật an toàn.
